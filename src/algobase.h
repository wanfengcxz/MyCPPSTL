//
// Created by 晚风吹行舟 on 2023/4/14.
//

#ifndef MYCPPSTL_ALGOBASE_H
#define MYCPPSTL_ALGOBASE_H
...
/*****************************************************************************************/
    ...
    OutputIter unchecked_copy_cat(InputIter first, InputIter last,
                                  OutputIter result, stl::input_iterator_tag) {
    ...
    OutputIter unchecked_copy_cat(RandomIter first, RandomIter last,
                                  OutputIter result, stl::random_access_iterator_tag) {
...
/*****************************************************************************************/
    ...
    unchecked_copy_backward_cat(BidirectionalIter1 first, BidirectionalIter1 last,
                                BidirectionalIter2 result, stl::bidirectional_iterator_tag) {
        // bidirectional支持双向移动
        while (first != last)
            *--result = *--last;
        return result;
    ...
    unchecked_copy_backward_cat(RandomIter1 first, RandomIter1 last,
                                BidirectionalIter2 result, stl::random_access_iterator_tag) {
...
/*****************************************************************************************/
    ...
    stl::pair<InputIter, OutputIter> unchecked_copy_n(InputIter first, Size n, OutputIter result,
                                                      stl::input_iterator_tag) {
        // 最普通的iterator，仅支持++
        for (; n > 0; --n, ++first, ++result) {
            *result = *first;
        }
        return stl::pair<InputIter, OutputIter>(first, result);
    }

    template<class RandomIter, class Size, class OutputIter>
    stl::pair<RandomIter, OutputIter>
    unchecked_copy_n(RandomIter first, Size n, OutputIter result,
                     stl::random_access_iterator_tag) {
        // random_access_iterator可以随机访问，所以可以直接+n
        auto last = first + n;
        return stl::pair<RandomIter, OutputIter>(last, stl::copy(first, last, result));
    }

    template<class InputIter, class Size, class OutputIter>
    stl::pair<InputIter, OutputIter> copy_n(InputIter first, Size n,
                                            OutputIter result) {
...
/*****************************************************************************************/
    ...
    unchecked_move_backward_cat(RandomIter1 first, RandomIter1 last,
                                RandomIter2 result, stl::random_access_iterator_tag) {
        for (auto n = last - first; n > 0; --n)
            *--result = stl::move(*--last);
        return result;
    }
...
/*****************************************************************************************/
    ...
    // 检查 T 是否为整数类型。若 T 为 bool 、 char 、 char8_t 、
    // char16_t 、 char32_t 、 wchar_t 、 short 、 int 、 long 、
    // long long 类型，或任何实现定义的扩展整数类型，包含任何有符号、无符号
    // 及 cv 限定的变体。则提供等于 true 的成员常量 value 。否则， value 等于 false 。
    typename std::enable_if<
...
/*****************************************************************************************/
// lexicographical_compare 小于ture 大于等于false
...
/*****************************************************************************************/
    ...
    bool lexicographical_compare(const unsigned char *first1,
        ...
        // 先比较相同长度的部分
        const auto result = std::memcmp(first1, first2, stl::min(len1, len2));
        // 若相等，长度较长的比较大
...
/*****************************************************************************************/
    template<class InputIter1, class InputIter2>
    stl::pair<InputIter1, InputIter2>
    mismatch(InputIter1 first1, InputIter1 last1, InputIter2 first2) {
        while (first1 != last1 && *first1 == *first2) {
            ++first1;
            ++first2;
        }
        return stl::pair<InputIter1, InputIter2>(first1, first2);
    }
